...
accumulators.$push = function (pushValue, previousArray, doc) {
  if (!previousArray || !Array.isArray(previousArray)) previousArray = [];   // Initialize the array property in case it is not
  // Check value of pushValue
  if (typeof pushValue === 'string' && pushValue.charAt(0) === '$'){   // Expression

  }
  else if (typeof pushValue === 'object'){

  }
  else{   // Other primitive value
